...
TEST(CheckCollectionDetails, Success) {
    ...
    quick_gzip_write(path + ".gz", payload1 + "\t12\n" + payload2 + "\t55\n");
...
TEST(CheckCollectionDetails, Failure) {
    ...
    quick_text_write(path, "aaron's\tcollection\tthis is aaron's collection\t12345\tAaron Lun\thttps://aaron.net\n" + payload2 + "\n");
    quick_gzip_write(path + ".gz", payload1 + "\t12\n" + payload2 + "\t55\n");
    expect_error([&]() { gesel::internal::check_collection_details(path, std::vector<uint64_t>{ r1, r2 }, std::vector<uint64_t>{ 12, 55 }); }, "non-digit");

    quick_text_write(path, "aaron's collection\tthis is aaron's collection\tAARON\tAaron Lun\thttps://aaron.net\n" + payload2 + "\n");
    quick_gzip_write(path + ".gz", payload1 + "\t12\n" + payload2 + "\t55\n");
    expect_error([&]() { gesel::internal::check_collection_details(path, std::vector<uint64_t>{ r1, r2 }, std::vector<uint64_t>{ 12, 55 }); }, "non-digit");

    quick_text_write(path, "aaron's collection\tthis is aaron's collection\t12345\tAaron Lun\thttps://aaron.net\tfoo\n" + payload2 + "\n");
    quick_gzip_write(path + ".gz", payload1 + "\t12\n" + payload2 + "\t55\n");
