...
inline std::vector<std::pair<int32_t, int32_t> > load_ranges_with_sizes(const std::string& path) {
    byteme::GzipFileReader reader(path);
    byteme::PerByte pb(&reader);
    ...
    while (valid) {
        ...
        ++line;
        output.emplace_back(byte_size, other_size);
    }

    return output;
}

inline std::vector<std::pair<std::string, int32_t> > load_named_ranges(const std::string& path) {
    byteme::GzipFileReader reader(path);
    byteme::PerByte pb(&reader);
    std::vector<std::pair<std::string, int32_t> > output;
    ...
    while (valid) {
        ...
        int32_t other_size = parse_integer_field<FieldType::LAST>(pb, valid, path, line);
        ++line;
        output.emplace_back(name, other_size);
    }
